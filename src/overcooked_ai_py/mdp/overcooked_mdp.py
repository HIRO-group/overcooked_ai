...
class OvercookedGridworld(object):
    ...
    def get_fully_random_start_state_fn(self, mlam):
        def start_state_fn(random_pos=False, random_dir=False, max_random_objs=0):
            ...
            start_state = OvercookedState.from_player_positions(start_pos, bonus_orders=self.start_bonus_orders, all_orders=self.start_all_orders, random_orientation=random_dir)

            if max_random_objs <= 0:
                return start_state
            ...
            for counter_idx in counter_indices:
                ...
                if obj == "soup":
                    ...
                    obj = ObjectState(obj, counter_pos)
                start_state.add_object(obj, counter_pos)
    ...
    def get_subtask_start_state_fn(self, mlam):
        def start_state_fn(p_idx=0, curr_subtask='unknown', random_pos=False, random_dir=False,
            ...
            if curr_subtask in ['get_onion_from_dispenser', 'get_plate_from_dish_rack',
                ...
                if 'from_counter' in curr_subtask:
                    ...
                    if curr_subtask == 'get_onion_from_counter':
                        obj_name = 'onion'
                    elif curr_subtask == 'get_plate_from_counter':
                        obj_name = 'dish'
                    elif curr_subtask == 'get_soup_from_counter':
                        obj_name = 'soup'
                    possible_counters = self.find_free_counters_valid_for_player(start_state, mlam, p_idx)
            ...
            elif curr_subtask in ['put_soup_closer', 'serve_soup']:
                player.set_object(SoupState.get_soup(player.position, num_onions=3, finished=True))
            ...
            # Randomize held items
            # What the curr_subtask agent is carrying is already decided. Only randomly assign other agent random object
    ...
    def find_free_counters_valid_for_player(self, state, mlam, p_idx):
        """Finds all empty counter locations that are accessible to both players"""
        ...
        for free_counter in free_counters:
            goals = mlam.motion_planner.motion_goals_for_pos[free_counter]
            if any([mlam.motion_planner.is_valid_motion_start_goal_pair(player.pos_and_or, goal) for goal in goals]):
                free_counters_valid_for_both.append(free_counter)
        return free_counters_valid_for_both
    ...
    def lossless_state_encoding(self, overcooked_state, goal_objects=None, horizon=400, p_idx=None, debug=False):
        ...
        def process_for_player(primary_agent_idx):
            ...
            ordered_player_features.append("ego_agent_loc")
            ordered_player_features.append("teammates_loc")
            ordered_player_features.extend([
                f"ego_agent_orientation_{Direction.DIRECTION_TO_INDEX[d]}"
                for d in Direction.ALL_DIRECTIONS
            ])
            ordered_player_features.extend([
                f"teammates_orientation_{Direction.DIRECTION_TO_INDEX[d]}"
                for d in Direction.ALL_DIRECTIONS
            ])
            ...
            if goal_objects == "counters":
                for loc in self.get_empty_counter_locations(overcooked_state):
            ...
            for i, player in enumerate(overcooked_state.players):
                ...
                if i == primary_agent_idx:
                    state_mask_dict["ego_agent_loc"][player.position] = 1
                    state_mask_dict[f"ego_agent_orientation_{player_orientation_idx}"][player.position] = 1
                else:
                    state_mask_dict["teammates_loc"][player.position] = 1
                    state_mask_dict[f"teammates_orientation_{player_orientation_idx}"][player.position] = 1
